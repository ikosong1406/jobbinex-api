...
// GET endpoint to get current subscription status
router.get("/", async (req, res) => {
  ...
  try {
    const user = await User.findById(userId)
      .select("plan assistant")
      .populate({
        path: "assistant",
        select: "firstname lastname specialization email status"
      });
...
// DELETE endpoint to cancel subscription (optional)
router.delete("/", async (req, res) => {
  ...
  try {
    ...
    try {
      await session.withTransaction(async () => {
        // Get user to find assigned assistant
        const user = await User.findById(userId).session(session);
        if (!user) {
          throw new Error("User not found");
        }

        const assistantId = user.assistant;

        // Remove user's plan and assistant
        const updatedUser = await User.findByIdAndUpdate(
          userId,
          { 
            $unset: { 
        ...
        ).select("-password -jobPassword");

        // Remove user from assistant's clients if assistant exists
        if (assistantId) {
          await Assistant.findByIdAndUpdate(
            assistantId,
            { 
